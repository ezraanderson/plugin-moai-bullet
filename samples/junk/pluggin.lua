...
--THE : WORLD
  ...
  bWorld:start() 
      ...
      --DEBUG ON LAYER : MAYBE SHOULD TRY SOMETHING DIFFERENT
...
--***********************************************************************************
      ...
      --pos_start:setIdentity();pos_start:setEulerZYX(0,0,0);pos_start:setOrigin(0,0,0)   
      --local car = bWorld:newBody (pos_start)
      --car:addCar(0,0,0)
...
---**************************************************************************************              
          ...
          local plane_mesh= MOAIMesh.new ()
                plane_mesh:setTexture ( "grass.png" )
...
local function make_fog()
        local plane = {}

 print('-------------------------------------------------')    
local slide = 25 -- =45 WTF ???????????????
print('>>>>',math.deg(slide))
...
--gfxQuad:setTexture ("fog.png" )
--gfxQuad:setRect ( -25, -25, 25, 25)



               fog_prop = MOAIProp.new ()
--                fog_prop:setDeck ( gfxQuad )
--                --fog_prop:setTexture ( "fog.png" )
                
--               -- fog_prop:setShader ( MOAIShaderMgr.getShader ( MOAIShaderMgr.MESH_SHADER ))
--               -- plane_prop:setCullMode ( MOAIProp.CULL_BACK )
--               -- fog_prop:setDepthTest(MOAIProp.DEPTH_TEST_LESS_EQUAL)
--             --   fog_prop:setDepthMask(true)               
--                fog_prop:setLoc(0,0,-1000)         
--                fog_prop:setColor(0.7,0.7,0.7,0.7)
...
--circle['body']:addToWorld(false)
--circle['body']:setRestitution(0)   

--pos_start:setIdentity();pos_start:setEulerZYX(0,0,0);pos_start:setOrigin(0,0,0); 
----ONE SHAPE TO BODIES
--circle['shapeA'] = circle['body']:newShape(pos_start)
--circle['shapeA']:makeSphere(.1,.1)  
...
local function rag_doll()
    ...
    m_body['BODYPART_PELVIS']:setActivationState(1)   
    ...
    m_body['BODYPART_SPINE']:setActivationState(1)    
    ...
    m_body['BODYPART_HEAD']:setActivationState(1)  
    ...
    m_body['BODYPART_LEFT_UPPER_LEG']:setActivationState(1) 
    ...
    m_body['BODYPART_RIGHT_UPPER_LEG']:setActivationState(1)   
    ...
    m_body['BODYPART_LEFT_UPPER_ARM']:setActivationState(1)   
    ...
    m_body['BODYPART_RIGHT_UPPER_ARM']:setActivationState(1) 
    ...
    --*******************************
    btTransformA:setIdentity();btTransformA:setEulerZYX(M_PI_2,M_PI_2,M_PI_2);btTransformA:setOrigin(M_PI_2,M_PI_2, M_PI_2);
    btTransformB:setIdentity();btTransformB:setEulerZYX(M_PI_2,M_PI_2,M_PI_2);btTransformB:setOrigin(M_PI_2,M_PI_2,M_PI_2);
    
    print("ABOUT TO CRASHE");
    
    m_joints['JOINT_PELVIS_SPINE'] = bWorld:addJointHinge(m_body['BODYPART_PELVIS'],m_body['BODYPART_SPINE'],btTransformA,btTransformB)    
    print( m_joints['JOINT_PELVIS_SPINE']);   
...
----************************************************************************************
----HEAD
 m_mesh['head'] = dofile('head.lua')
 m_mesh['head']:setTexture ( "face.png" )
...
m_mesh['leg_left'] = dofile('leg.lua')
m_mesh['leg_left'] :setTexture ( "leg.png" )
...
----************************************************************************************
----leg_right
m_mesh['leg_right'] = dofile('leg.lua')
m_mesh['leg_right'] :setTexture ( "leg.png" )
...
----************************************************************************************
----ARM_RIGHT
m_mesh['arm_right'] = dofile('arm.lua')
m_mesh['arm_right'] :setTexture ( "arm.png" )
...
--************************************************************************************
--ARM_LEFT
m_mesh['arm_left'] = dofile('arm.lua')
m_mesh['arm_left'] :setTexture ( "arm.png" )
...
----************************************************************************************
----SPINE
m_mesh['spine'] = dofile('spine.lua')
m_mesh['spine'] :setTexture ( "spine.png" )
...
---JOINT RAG TO CAR
    ...
    m_joints['bum'] = bWorld:addJointCone(m_body['BODYPART_PELVIS'],m_body['car'],btTransformA,btTransformB)
    ...
    m_joints['leg_left'] = bWorld:addJointCone(m_body['BODYPART_HEAD'],m_body['car'],btTransformA,btTransformB)
    m_joints['leg_left']:setBreakingImpulse(1.3)
    
    
    
     m_joints['bum']:setEnabled(false)
...
--MOTION
    ...
    if keyboard:keyIsDown ( 's' ) then      
        ...
        --m_vehicle['car']:applyBrake(10,3) 
        --m_vehicle['car']:applyBrake(10,4)   
...
--print('function _screenSize',myDisplay['width'] ,	myDisplay['height'])
